...
- `score` is optional and nullable; when supplied it must be an integer from 20 through 200.
...
- `POST /api/v1/admin/course-photos/{image_id}/feature` — body `{"featured": true|false}`. Featuring approves the photo if needed, sets `is_hero`, and clears `is_hero` on every other photo of the same course **in the same source tier**; other tiers are untouched. Sets `moderation_action` to `featured` (or `unfeatured` when clearing).
