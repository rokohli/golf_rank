...
describe('RatingFlow', () => {
  ...
  it('shares a new rating publicly by default', async () => {
    render(<RatingFlow {...props({ getCandidate: jest.fn().mockResolvedValue(null) })} />)
    await chooseTierAndOpenRound()

    fireEvent.press(screen.getByRole('button', { name: 'Friends' }))
    expect(screen.getByRole('button', { name: 'Set visibility to public' }).props.accessibilityState).toMatchObject({ selected: true })
  })
  ...
  it('respects an existing private round instead of overriding it with the new-rating default', async () => {
    ...
    fireEvent.press(screen.getByRole('button', { name: 'Friends' }))
    expect(screen.getByRole('button', { name: 'Set visibility to private' }).props.accessibilityState).toMatchObject({ selected: true })
  })

  it('respects an existing friends-only round and does not collapse it to public or private', async () => {
    const friendsOnlyRating: CourseRatingState = { ...existingRating, round: { ...existingRating.round!, visibility: 'friends' } }
    render(<RatingFlow {...props({ initialRating: friendsOnlyRating })} />)
    await openExistingRound()

    fireEvent.press(screen.getByRole('button', { name: 'Friends' }))
    expect(screen.getByRole('button', { name: 'Set visibility to friends' }).props.accessibilityState).toMatchObject({ selected: true })
  })

  it('does not broaden a friends-only round to public when an unrelated detail changes', async () => {
    const friendsOnlyRating: CourseRatingState = { ...existingRating, round: { ...existingRating.round!, visibility: 'friends' } }
    const inputProps = props({ initialRating: friendsOnlyRating })
    render(<RatingFlow {...inputProps} />)
    await openExistingRound()

    fireEvent.press(screen.getByRole('button', { name: 'Notes' }))
    fireEvent.changeText(screen.getByLabelText('Round notes'), 'Played with the whole group today')
    fireEvent.press(screen.getByRole('button', { name: 'Continue' }))

    await waitFor(() => expect(inputProps.saveDetails).toHaveBeenCalledWith(expect.objectContaining({ visibility: 'friends' })))
  })
