...
import { getCourse, getCourseRating } from '../../src/api/client'
...
import { Course, CourseRatingState } from '../../src/types'
  ...
  return <>
    ...
    <ProductScreen>
      ...
      <View style={styles.actions}>
        ...
        <CourseAction icon="heart" label="Save" />
...
function CourseAction({ icon, label, onPress }: { icon: keyof typeof Feather.glyphMap; label: string; onPress?: () => void }) {
  return <Pressable accessibilityLabel={label} accessibilityRole="button" onPress={onPress} style={styles.action}><View style={styles.actionIcon}><Feather name={icon} size={18} color={colors.pine} /></View><Text style={styles.actionLabel}>{label}</Text></Pressable>
}
