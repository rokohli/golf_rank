  ...
  return <>
    ...
    <ProductScreen refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadFollows(true)} tintColor={colors.pine} />}>
      ...
      {searching ? <View style={styles.searchSection}>
        ...
        {results.map((user) => <UserRow key={user.id} user={user} detail={user.home_region ?? `@${user.username ?? 'golfer'}`} action={follows.some((item) => item.user.id === user.id) ? 'Following' : 'Follow'} busy={busyUserId === user.id} onAction={() => void toggleFollow(user)} onOpen={() => openUserProfile(router, user.id)} />)}
      </View> : <>
