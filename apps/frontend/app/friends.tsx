...
import { blockUser, followUser, getFollows, getProfile, getSuggestedUsers, muteUser, searchUsers, unfollowUser } from '../src/api/client'
...
import { Follow, OnboardingPreferences, UserSummary } from '../src/types'
...
export default function Friends() {
  ...
  const loadFollows = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const [nextFollows, nextSuggested, nextProfile] = await Promise.all([
        getFollows(headers),
        getSuggestedUsers(headers).catch(() => []),
  ...
  const unFollowedSuggested = suggested.filter((item) => !follows.some((f) => f.user.id === item.id))
  ...
  return <>
    ...
    <ProductScreen refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadFollows(true)} tintColor={colors.pine} />}>
      ...
      {searching ? <View style={styles.searchSection}>
        <View style={styles.searchWrap}><Feather name="search" size={16} color={colors.muted} /><TextInput accessibilityLabel="Search golfers" autoFocus value={query} onChangeText={setQuery} placeholder="Name, username, course, or region" placeholderTextColor={colors.muted} style={styles.searchInput} /></View>
        {profile?.home_region || profile?.onboarding_data?.home_course_search ? (
          <View style={styles.quickChips}>
            {profile?.home_region ? (
              <Pressable
                accessibilityLabel={`Search near ${profile.home_region}`}
                accessibilityRole="button"
                onPress={() => setQuery(profile.home_region)}
                style={({ pressed }) => [styles.quickChip, pressed && styles.pressed]}
              >
                <Feather name="map-pin" size={12} color={colors.pine} />
                <Text style={styles.quickChipText}>Near {profile.home_region}</Text>
              </Pressable>
            ) : null}
            {profile?.onboarding_data?.home_course_search ? (
              <Pressable
                accessibilityLabel={`Search at ${profile.onboarding_data.home_course_search}`}
                accessibilityRole="button"
                onPress={() => setQuery(profile.onboarding_data!.home_course_search)}
                style={({ pressed }) => [styles.quickChip, pressed && styles.pressed]}
              >
                <Feather name="flag" size={12} color={colors.pine} />
                <Text style={styles.quickChipText}>At {profile.onboarding_data.home_course_search}</Text>
              </Pressable>
            ) : null}
          </View>
        ) : null}
        ...
        {results.map((user) => <UserRow key={user.id} user={user} detail={userDetail(user)} action={follows.some((item) => item.user.id === user.id) ? 'Following' : 'Follow'} busy={busyUserId === user.id} onAction={() => void toggleFollow(user)} onOpen={() => openUserProfile(router, user.id)} />)}
        {!searchLoading && query.trim().length === 0 && unFollowedSuggested.length > 0 ? (
          <View style={styles.suggestedSection}>
            <Text style={styles.sectionHeading}>Suggested golfers near you</Text>
            {unFollowedSuggested.map((user) => (
              <UserRow
                key={user.id}
                user={user}
                detail={userDetail(user)}
                action={follows.some((item) => item.user.id === user.id) ? 'Following' : 'Follow'}
                busy={busyUserId === user.id}
                onAction={() => void toggleFollow(user)}
                onOpen={() => openUserProfile(router, user.id)}
              />
            ))}
          </View>
        ) : null}
        ...
        {!loading && !follows.length ? <View style={styles.state}>
          <Feather name="user-plus" size={25} color={colors.muted} />
          <Text style={styles.emptyTitle}>Find your golf people</Text>
          <Text style={styles.empty}>Search for golfers to start building your feed.</Text>
          <Pressable accessibilityRole="button" onPress={() => setSearching(true)} style={styles.primary}>
            <Text style={styles.primaryText}>Find golfers</Text>
          </Pressable>
          {unFollowedSuggested.length > 0 ? (
            <View style={[styles.suggestedSection, { width: '100%', marginTop: 24 }]}>
              <Text style={styles.sectionHeading}>Suggested golfers near you</Text>
              {unFollowedSuggested.slice(0, 5).map((user) => (
                <UserRow
                  key={user.id}
                  user={user}
                  detail={userDetail(user)}
        ...
        <View>{follows.map((follow) => (
          <View key={follow.user.id} style={styles.followRow}>
            <UserRow
              user={follow.user}
              detail={follow.is_mutual ? 'Friends · mutual follow' : userDetail(follow.user, 'Following')}
              action="Unfollow"
              busy={busyUserId === follow.user.id}
              onAction={() => void toggleFollow(follow.user)}
              onOpen={() => openUserProfile(router, follow.user.id)}
              onMore={(anchor) => setSheet({ kind: 'menu', follow, anchor })}
            />
          </View>
        ))}</View>
        {!loading && follows.length > 0 && unFollowedSuggested.length > 0 ? (
          <View style={[styles.suggestedSection, { marginTop: 24 }]}>
            <Text style={styles.sectionHeading}>Suggested golfers near you</Text>
            {unFollowedSuggested.slice(0, 5).map((user) => (
              <UserRow
                key={user.id}
...
function UserRow({ user, detail, action, busy, onAction, onOpen, onMore }: {
  user: UserSummary
  detail: string
  action: string
  busy: boolean
  ...
  return (
    <View style={styles.row}>
      <Pressable accessibilityRole="button" accessibilityLabel={`Open ${user.display_name}'s profile`} onPress={onOpen} style={styles.identityPressable}>
...
function userDetail(user: UserSummary, fallback?: string): string {
  if (user.home_course_name && user.home_region) {
    return `${user.home_course_name} · ${user.home_region}`
  }
  if (user.home_course_name) {
    return user.home_course_name
  }
  return user.home_region ?? fallback ?? `@${user.username ?? 'golfer'}`
}
...
const styles = StyleSheet.create({
  ...
  quickChips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 4, marginBottom: 8 },
  quickChip: { alignItems: 'center', backgroundColor: colors.card, borderColor: colors.line, borderRadius: radii.pill, borderWidth: 1, flexDirection: 'row', gap: 6, paddingHorizontal: 12, paddingVertical: 6 },
  quickChipText: { color: colors.ink, fontSize: 12, fontWeight: '600' },
  pressed: { opacity: 0.7 },
  suggestedSection: { marginTop: 12 },
  sectionHeading: { color: colors.ink, fontFamily: 'Georgia', fontSize: 15, marginBottom: 8 },
  ...
  followRow: { borderBottomColor: colors.line, borderBottomWidth: StyleSheet.hairlineWidth }, row: { alignItems: 'center', flexDirection: 'row', gap: 11, paddingVertical: 13 }, identityPressable: { alignItems: 'center', flex: 1, flexDirection: 'row', gap: 11 }, name: { color: colors.ink, fontSize: 13, fontWeight: '800' }, meta: { color: colors.muted, fontSize: 10, marginTop: 4 }, followButton: { alignItems: 'center', borderColor: colors.pine, borderRadius: 16, borderWidth: 1, minWidth: 72, paddingHorizontal: 11, paddingVertical: 7 }, followingButton: { backgroundColor: '#EDF1ED' }, followButtonText: { color: colors.pine, fontSize: 10, fontWeight: '800' }, moreButton: { paddingHorizontal: 2, paddingVertical: 6 },
  menuRoot: { flex: 1 },
  menuCard: {
