    ...
    <ProductScreen>
      ...
      {!loading && round ? <>
        <Pressable accessibilityLabel={`Open ${round.course.name}`} accessibilityRole="button" onPress={() => router.push(`/course/${round.course.id}` as never)} style={({ pressed }) => [styles.feature, pressed && styles.pressed]}>
          ...
          <View style={styles.scoreBlock}><View style={styles.scoreValue}><Text accessibilityLabel={scoreAccessibilityLabel(round.score, round.course.par, 'Round score')} style={styles.score}>{round.score ?? '—'}</Text>{scoreToPar(round.score, round.course.par) ? <View style={styles.toParBadge}><Text style={styles.toPar}>{scoreToPar(round.score, round.course.par)}</Text></View> : null}</View><Text style={styles.scoreLabel}>Round score</Text></View>
        </Pressable>
