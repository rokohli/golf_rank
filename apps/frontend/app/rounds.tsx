...
export default function Rounds() {
  ...
  return <>
    ...
    <ProductScreen refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void load(filter, true)} tintColor={colors.pine} />}>
      ...
      <View style={styles.list}>{rounds.map((round) => <RoundRow key={round.id} round={round} onPress={() => router.push(`/round/${round.id}` as never)} />)}</View>
...
function RoundRow({ round, onPress }: { round: GolfRound; onPress: () => void }) {
  return <Pressable accessibilityLabel={`Open ${round.course.name} round`} accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.roundRow, pressed && { opacity: 0.6 }]}>
    <View style={{ flex: 1, gap: 4 }}><Text style={styles.date}>{formatDate(round.played_on)}</Text><Text style={styles.course}>{round.course.name}</Text><Text numberOfLines={1} style={styles.meta}>{round.course.region}{round.is_rating_round ? ' · Rating visit' : ''}</Text></View>
...
const styles = StyleSheet.create({
  ...
  roundRow: { alignItems: 'center', borderBottomColor: colors.line, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', gap: 10, paddingVertical: 14 },
  ...
  score: { color: colors.pine, fontSize: 26, fontWeight: '500', minWidth: 34, textAlign: 'right' },
